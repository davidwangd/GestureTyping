...
char *wordlist2[5] = {"hello", "world", "888888888", "9999", "00000000010000\n0000020000000003"};
...
int main() {
    ...
    while (!main_disp.is_closed()) {
        ...
        if (main_disp.button() && main_disp.mouse_x() >= 0 && main_disp.mouse_y()>=0) {
            ...
            board.setPosXY(x, y);
            if (cnt == 100) board.setGesture(2);
            else if (cnt == 200) {
                board.setwords(wordlist, 5, true);
            }
            else if (cnt == 300) board.setGesture(1);
            else if (cnt == 400) {
                board.setwords(wordlist2, 5, true);
                cnt = 0;
            }
        }
    }
